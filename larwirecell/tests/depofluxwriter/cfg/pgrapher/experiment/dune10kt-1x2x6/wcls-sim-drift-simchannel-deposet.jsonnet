...
// local graph = g.pipeline([wcls_input.deposet, setdrifter, wcls_simchannel_sink, bagger, bi_manifold, retagger, wcls_output.sim_digits, sink]);

//FIXME: need a node to trim the deposet with a time window as "bagger" did
local graph = g.pipeline([wcls_input.deposet, setdrifter, wcls_simchannel_sink, bi_manifold, retagger, wcls_output.sim_digits, sink]);
